...
func (p *proxy)Run(cmd string, timeout time.Duration) (outStr string) {
	outStr, _, _, err := p.ssh.Run(cmd, timeout)
	if err != nil {
		log.Fatalln("error while getMetaData. Error: ", err.Error())
	}
	return outStr
}

func (p *proxy)getCredentialsFromEC2()  {
	stdout, stderr, done, err := p.ssh.Run("curl http://169.254.169.254/latest/meta-data/iam/security-credentials/",120 * time.Second)
	if err != nil {
		panic("Can't run remote command: " + err.Error())
	} else {
		fmt.Println("don is :", done, "stdout is :", stdout, ";   stderr is :", stderr)
	}
	roleName := p.Run("curl http://169.254.169.254/latest/meta-data/iam/security-credentials/",120 * time.Second)
	metaDataStr := p.Run("curl http://169.254.169.254/latest/meta-data/iam/security-credentials/" + roleName,120 * time.Second)
    log.Println(metaDataStr)
    var metaDataStruct = &metaData{}
	json.Unmarshal([]byte(metaDataStr),metaDataStruct)
	os.Setenv("AWS_ACCESS_KEY_ID",metaDataStruct.AccessKeyId)
	os.Setenv("AWS_SECRET_ACCESS_KEY",metaDataStruct.SecretAccessKey)
	os.Setenv("AWS_SESSION_TOKEN",metaDataStruct.Token)
}
...
func setupCredentialsEndpoints() (endpoints.Resolver, func()) {
...
func main() {
	...
	flag.StringVar(&sshYamlConfig,"ssh-config","", "Access to ec2 machine information")
	...
	ssh := &easyssh.MakeConfig{}
	yamlFile, err := ioutil.ReadFile(sshYamlConfig)
	if err != nil {
		log.Fatalf("yamlFile.Get err   #%v ", err)
	}
	err = yaml.Unmarshal(yamlFile, ssh)
	if err != nil {
